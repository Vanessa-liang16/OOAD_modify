...
public class FuncPanelHandler extends PanelHandler
{
	...
	{
		int clickedIndex = btnlist.indexOf(e.getSource());
		// System.out.println("按鈕被點擊: index = " + clickedIndex);
    	setCurrentBtn(clickedIndex);
	}

	public void setCurrentBtn(int index)
	{
		// System.out.println("設定當前按鈕: " + index);
	...
	ImageIcon getIcon(int index)
	{
		switch (index)
		{
			...
			//ADD
			case 6:
				return new ImageIcon("icon/dependency_link.jpg");
			default:
				return null;
		}
	}
	...
	{
		// System.out.println("getFunc 被呼叫，index = " + index);
		switch (index)
		{
			case 0:
